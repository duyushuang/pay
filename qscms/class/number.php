...
class number{
	...
	public static function chinese($number){
	   ...
	   for(self::$level=0;$number>0.0001;self::$level++,$number=floor($number / 10000)){
		   ...
		   if($n4) {
			   isset($parsed[self::$level]) || $parsed[self::$level] = '';
		   ...
		   if($n3) {
			  ...
			  if(isset($parsed[self::$level]) && !preg_match("/零$/",$parsed[self::$level]) && ($number / 1000)>=1) {// 不出现连续两个“零”的情况
			  	isset($parsed[self::$level]) || $parsed[self::$level] = '';
				 $parsed[self::$level].="零";
			  }
		   }
		   if($n2) {
			  ...
			  if(isset($parsed[self::$level]) && !preg_match("/零$/", $parsed[self::$level]) && ($number / 100)>=1) {// 不出现连续两个“零”的情况
			  	isset($parsed[self::$level]) || $parsed[self::$level] = '';
				 $parsed[self::$level].="零";
			  }
		   }
	   ...
	   for(self::$level-=1;self::$level>=0;self::$level--){
		   $result.= (isset($parsed[self::$level]) ? $parsed[self::$level] : '').(isset(self::$top[self::$level]) ? self::$top[self::$level] : '');
	   }
