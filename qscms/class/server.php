...
class server{
	...
	public static function dirfile_check(&$dirfile_items) {
		...
		foreach($dirfile_items as $key => $item) {
			$item_path = $item['path'];
			if($item['type'] == 'dir') {
				if(!self::dir_writeable($item_path)) {
					if(is_dir($item_path)) {
				...
				if(file_exists($item_path)) {
					...
					if(self::dir_writeable(dirname($item_path))) {
						$dirfile_items[$key]['status'] = 1;
						$dirfile_items[$key]['current'] = '+r+w';
					} else {
						$dirfile_items[$key]['status'] = -1;
		...
		foreach($env_items as $key => $item) {
			if($key == 'php') {
				...
				if(function_exists('disk_free_space')) {
					$env_items[$key]['current'] = floor(disk_free_space(WROOT) / (1024*1024)).'M';
				} else {
			...
			if($item['r'] != 'notset' && string::getSize($env_items[$key]['current']) < string::getSize($item['r'])) {
