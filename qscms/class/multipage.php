...
class multipage{
	...
	public static function parse($pagecount,$pagesize,$page,$url_list,$style,$ppagesize=10, $saveName = ''){
		...
		if($useppage){
			...
			//$pagestart >= $minpage + $fillSize && $page == $pagestart && ($pagestart -= $fillSize);
		...
		$cache_file = self::$saveDir.$saveName.'.php';
		if(!file_exists($cache_file)){
			$style=str_replace('"','\"',$style);
			$style=str_replace('{weburl}', WEB_URL, $style);
			...
			$style=preg_replace('/({.*?)minppage(.*?})/','$1$minppage$2',$style);
			$style=preg_replace('/({.*?)maxppage(.*?})/','$1$maxppage$2',$style);
			//$style=preg_replace('/({.*?)page(.*?})/','$1$page$2',$style);
			$style=str_replace('{$minpage}','".$minpage."',$style);
			$style=str_replace('{$maxpage}','".$maxpage."',$style);
			$style=str_replace('{$minppage}','".$minppage."',$style);
			$style=str_replace('{$maxppage}','".$maxppage."',$style);
			$style=str_replace('{page}','".$page."',$style);
			$style = preg_replace('/\{page\s*(\+|-)\s*(.+?)\}/s', '".(\$page$1$2)."', $style);
			$style = preg_replace('/\(\$page(\+|-)(minpage|maxpage|minppage|maxppage)\)/s', '($page$1\$$2)', $style);
			$style=str_replace('{count}','".$pagecount."',$style);
			$style=str_replace('{pagesize}','".$pagesize."',$style);
			//$style=str_replace('$min$page','".$minpage."',$style);
			//$style=str_replace('$max$page','".$maxpage."',$style);
			$style = str_replace('{url}', '".$baseUrl."', $style);
			$style=preg_replace("/({url.*?)page(.*?})/",'$1$page$2',$style);
			$style=str_replace('$min$page','$minpage',$style);
			$style=str_replace('$max$page','$maxpage',$style);
			$style=str_replace('$min$ppage','$minppage',$style);
			$style=str_replace('$max$ppage','$maxppage',$style);
			$style=preg_replace("/{page(>|<|=)(.+?)}(.*?){\\/page}/s",'";if($page$1$2){$html.="$3";};$html.="',$style);
			$style=preg_replace("/{ppage(>|<|=)(.+?)}(.*?){\\/ppage}/s",'";if($atpage$1$2){$html.="$3";};$html.="',$style);
...
multipage::$saveDir = d(qscms::getCfgPath('/system/cacheDirRoot+/system/cacheDirs/multipage'));
file::createFolder(multipage::$saveDir);
?>