...
class sms{
	...
	public static function onePrice($uid) {
		return cfg::getMoney('sms', 'price_one');
	}
	public static function sendPhone($phone, $message, $uid_ = 0){
		$timestamp = time::$timestamp;
		$uid_ || $uid_ = qscms::v('_G')->uid;
		if (form::check_mobilephone($phone)) {
			$vip = false;
			$memberFields = member_base::getMemberFields($uid_);
			if ($vip) {
				$onePrice = cfg::getMoney('sms', 'price_one_vip');
			} else {
				$onePrice = cfg::getMoney('sms', 'price_one_user');
			}
	...
	public static function getList($uid_ = 0, $spage = 0, $spagesize = 0){
		...
		$v = qscms::v('_G');
		$uid_ || $uid_ = $v->uid;
		$spage || $spage = $v->gp_page;
		$spagesize || $spagesize = $v->gp_pagesize;
