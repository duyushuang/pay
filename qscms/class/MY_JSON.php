...
class MY_JSON {
		...
		switch (strtolower($str)) {
			...
			default: 
				...
				if (is_numeric($str)) { 
					...
					if ($str{0} == '[') {
						 $stk = array(MY_JSON_IN_ARR);
						 $arr = array();
					} else {
						if ($this->use & MY_JSON_LOOSE_TYPE) {
							$stk = array(MY_JSON_IN_OBJ);
							$obj = array();
						} else {
							$stk = array(MY_JSON_IN_OBJ);
							$obj = array();
					...
					array_push($stk, array('what' => MY_JSON_SLICE, 'where' => 0, 'delim' => false));
					$chrs = substr($str, 1, -1);
					$chrs = $this->reduce_string($chrs);
					if ($chrs == '') {
						if (reset($stk) == MY_JSON_IN_ARR) { return $arr; } else { return $obj; }
					}
					$strlen_chrs = strlen($chrs);
					for ($c = 0; $c <= $strlen_chrs; ++$c) {
						$top = end($stk);
						$substr_chrs_c_2 = substr($chrs, $c, 2);
						if (($c == $strlen_chrs) || (($chrs{$c} == ',') && ($top['what'] == MY_JSON_SLICE))) {
							$slice = substr($chrs, $top['where'], ($c - $top['where'])); 
							array_push($stk, array('what' => MY_JSON_SLICE, 'where' => ($c + 1), 'delim' => false));
							if (reset($stk) == MY_JSON_IN_ARR) {
								array_push($arr, $this->decode($slice));
							} elseif (reset($stk) == MY_JSON_IN_OBJ) {
								$parts = array();
						...
						} elseif ((($chrs{$c} == '"') || ($chrs{$c} == "'")) && ($top['what'] != MY_JSON_IN_STR)) {
							array_push($stk, array('what' => MY_JSON_IN_STR, 'where' => $c, 'delim' => $chrs{$c}));
						} elseif (($chrs{$c} == $top['delim']) && ($top['what'] == MY_JSON_IN_STR) && ((strlen(substr($chrs, 0, $c)) - strlen(rtrim(substr($chrs, 0, $c), '\\'))) % 2 != 1)) {
							array_pop($stk);
						} elseif (($chrs{$c} == '[') && in_array($top['what'], array(MY_JSON_SLICE, MY_JSON_IN_ARR, MY_JSON_IN_OBJ))) {
							array_push($stk, array('what' => MY_JSON_IN_ARR, 'where' => $c, 'delim' => false));
						} elseif (($chrs{$c} == ']') && ($top['what'] == MY_JSON_IN_ARR)) {
							array_pop($stk);
						} elseif (($chrs{$c} == '{') && in_array($top['what'], array(MY_JSON_SLICE, MY_JSON_IN_ARR, MY_JSON_IN_OBJ))) {
							array_push($stk, array('what' => MY_JSON_IN_OBJ, 'where' => $c, 'delim' => false));
						} elseif (($chrs{$c} == '}') && ($top['what'] == MY_JSON_IN_OBJ)) {
							array_pop($stk);
						} elseif (($substr_chrs_c_2 == '/*') && in_array($top['what'], array(MY_JSON_SLICE, MY_JSON_IN_ARR, MY_JSON_IN_OBJ))) {
							array_push($stk, array('what' => MY_JSON_IN_CMT, 'where' => $c, 'delim' => false));
							$c++;
						} elseif (($substr_chrs_c_2 == '*/') && ($top['what'] == MY_JSON_IN_CMT)) {
							array_pop($stk);
							$c++;
	...
	//if (class_exists('pear')) { return PEAR::isError($data, $code); } else
	if (is_object($data) && (get_class($data) == 'MY_JSON_error' || is_subclass_of($data, 'MY_JSON_error'))) { return true; } return false; 
	}
} 

//if (class_exists('PEAR_Error')) { class MY_JSON_Error extends PEAR_Error { function MY_JSON_Error($message = 'unknown error', $code = null, $mode = null, $options = null, $userinfo = null) { parent::PEAR_Error($message, $code, $mode, $options, $userinfo); } } } else {
 class MY_JSON_Error { function MY_JSON_Error($message = 'unknown error', $code = null, $mode = null, $options = null, $userinfo = null) { } } 
 //} 
 ?>