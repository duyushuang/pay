...
class member extends ext_base{
	...
	function edit($datas) {
		...
		if ($arr) {
			if (db::update('member_count', $arr, "uid='$this->id'")) return true;
	...
	function load(){
		if ($line = db::one('member', '*', "id='$this->id'")) {
			...
			$c_line = db::one('member_count', '*', "uid='$this->id'");
			foreach($c_line as $k => $v){
				$this->{'_'.$k} = $v;
			}
	...
	function allowPutMoney(){
		...
		if ($everyDayHelpMoney > 0) {
			$todayHelpMoney = qscms::formatMoney(db::one_one('shop', 'SUM(allMoney)', "type=0 AND cid=0 AND isSys=0 AND status<>300 AND (addTime >= $start AND addTime < $end)"));
		...
		if ($this->_isDebug) return array('allow' => 999999999, 'used' => 0, 'leftOver' => 999999999, 'limitMoney' => 1000000);
		return array('allow' => $everyDayHelpMoney, 'used' => $todayHelpMoney, 'leftOver' => $everyDayHelpMoney - $todayHelpMoney, 'limitMoney' => $limitMoney);
	}
	function buy($datas){
		if (!$this->_isDebug) {
			if (time() < 1464480000) return '暂时不能提供帮助';
			if (!$this->checkSafePassword()) return '安全码错误，操作失败';
			
			$var = qscms::v('_G');
			//$start = $var->todayStart;
			//$end   = $var->todayEnd;
			$start = time::$todayStart;
			$end   = time::$todayEnd;
	...
	function sell($datas){
		if (time() < 1464480000) return '暂时不能得到帮助';
		$useCommit = true;
		$yu = $this->_money;//账户余额;
		if (!$this->_isDebug) {
			if (!$this->checkSafePassword()) return '安全码错误，操作失败';
		...
		$allMoney = $allYuMoney1 = 0;
		$shops = array();
		
		$useCommit && db::autocommit();
		
		foreach ($sell as $sid => $money) {
			if ($line = db::one('shop', 'lixi,lixiYu,is_used', "id='$sid' AND uid='$this->id' AND status='102' AND lixiStatus='1' AND isFreeze='0' AND isLock='0' AND cid=0 AND lixiYu>0 AND restartsys=0 AND (isSys='1' OR complateTime>0)", false, $useCommit)) {
		...
		$uMoney = db::one_one('shop', 'SUM(allMoney)', "type='0' AND isSys='0' AND uid='$this->id' AND status<='102' AND complateTime='0'");//所有排单没有完成的总金额
		if ($uMoney / $arr['yuMoney'] < 0.8){
			if ($uMoney >= 5000){
				$arr['yuMoney1'] = $arr['yuMoney'] * 50 / 100;
				$arr['yuMoney1'] = floor($arr['yuMoney1'] / $bei) * $bei;	
			}else{
				$arr['yuMoney1'] = $arr['yuMoney'] * mt_rand(30, 70) / 100;
				$arr['yuMoney1'] = floor($arr['yuMoney1'] / $bei) * $bei;
		...
		if ($allYuMoney1 > 0) {
			$arr['yuMoney1'] = $arr['yuMoney'] - $arr['useYu'] - $allYuMoney1;
		}
	...
	function getShops($pagesize, $page, $where = '', $hide = 0){
		...
		$wh = $wh1 = '';
		if ($hide == 2){
			$wh  = 'status not in(103, 202, 300, 400) AND ';
			$wh1 = 't0.status not in(103, 202, 300, 400) AND ';
		}
		if ($total = db::dataCount('shop', $wh."(type='0' OR type='1') AND uid='$this->id' AND isSys=0 AND cid=0")) {
			...
			$list = db::select("shop|member_count:uid=uid", '*|name,xin', $wh1."(t0.type='0' OR t0.type='1') AND t0.uid='$this->id' AND t0.isSys=0 AND t0.cid=0", 't0.id DESC', $pagesize, $page);
		}
	...
	function getOrders($pagesize, $page, $where = '', $hide = false){
		...
		$wh = $wh1 = '';
		if ($hide == 2) {
			$wh  = 'status not in(13, 20) AND ';
			$wh1 = 't0.status not in(13, 20) AND ';
		}//$wh = ' AND status not in(103, 202, 300, 400)'; 
		$uid = $this->id;
		if ($total = db::dataCount('shop_order', $wh."(fuid='$uid' OR tuid='$uid')")) {
			...
			$list = db::select("shop_order|member_count:uid=fuid|member_count:uid=t0.tuid", '*|name fname|name tname', $wh1."(t0.fuid='$uid' OR t0.tuid='$uid')", 't0.id DESC', $pagesize, $page);
		}
		//print_r($list);exit;
	...
	function getOrder($id){
		...
		return db::selectFirst("shop_order|member_count:uid=fuid|member_count:uid=t0.tuid", '*|name fname|name tname', "t0.id='$id' AND (t0.fuid='$uid' OR t0.tuid='$uid')");
	}
	...
	function getSellShop($order = false){
		...
		foreach (db::select('shop', '*', "uid='$this->id' AND status='102' AND lixiStatus='1' AND isFreeze='0' AND isLock='0' AND cid=0 AND restartsys=0 AND lixiYu>0 AND (isSys='1' OR complateTime>0)", $order) as $v) {
			if ($v['isSys'] && $v['fuid']) {
	...
	function isAllowSell(){
		...
		if ($this->_is_sys == 0){
			if (!db::dataCount('shop', "type='0' AND uid='$this->id' AND isSys='0' AND restartsys=0 AND status IN(102,103) AND (isSys='1' OR complateTime>0)")) return '<span style="font-weight: bold;">抱歉，但此刻您的马夫罗没有得到证实（或冻结）。</span>
	<br>
	因此，不可能提取。
	<br>
	为了证实自己的马夫罗，首先您必须有一个人帮助。';//'您还有捐助未完成';
			if ($one = db::one('shop', '*', "uid='$this->id' AND isSys=0 AND complateTime=0 AND(status=101 OR status=102)")){
				if ($one['allMoney'] == $one['yuMoney'] || ($one['buyTotal'] > 0 && !db::one('shop_order', "fsid='$one[id]' AND status=20"))){
					return '您必须支付预付款。';
				}
			}
			/*
	...
	function orderAgree($id){
		$useCommit = true;
		$useCommit && db::autocommit();
		if ($one = db::one('shop_order', '*', "id='$id' AND fuid='$this->id' AND status=10)", false, $useCommit)) {
			$time = time();
			if ($time <= $one['endTime']){
	...
	function orderConfirm($id){
		...
		if ($one = db::one('shop_order', '*', "id='$id' AND tuid='$this->id' AND status='12'", false, $useCommit)) {
			...
			if (db::update('shop_order', "status='20',comTime='$time'", "id='$id'")) {
				 ...
				 * 判断是否为抢单 是的话 返回激活码
				 */
				if ($one['qiang'] == 3) {//返回打款方激活码
					db::update('member', 'checkNum=checkNum+5', "id='$one[fuid]'");
				}
	...
	function shopInfo($id){
		if ($shop = db::one('shop', '*', "id='$id' AND uid='$this->id'")) {//先获取商品
			...
			if ($shop['type'] == 0) {//买 援助别人
				...
				$wh = "t0.fid='$shop[id]'";
				foreach (db::select("shop_sell|shop:id=sid", 'money|addTime', $wh) as $v) {
					$shops[] = array(
						'time' => $v['addTime'],
						'money' => $v['money'],
					);
			...
			$orders = db::select("shop_sell|member_count:uid=fuid|member_count:uid=t0.tuid", '*|name fname|name tname', $wh);
	...
	static function recover($datas){
		...
		if ($datas['user'] && $datas['vcode'] && $datas['password']){
			...
			if (form::checkEmail($datas['user']) && $one = db::one('member', 'id,salt,password,safePassword', "email='$datas[user]'")){
				...
				if ($sendLog = db::one('log_email_vcode', '*', "email='$datas[user]' AND hash='$hash'", 'time desc')) {
	 ...
	 * 抢单功能
	 */
	public function qiang($id){
		$var = qscms::v('_G');
		$t1 = $var->todayStart;
		$t2 = $var->todayEnd;
		if (!$this->_isDebug) {
			if (db::dataCount('shop_order', "fuid='$this->id' AND status=20") == 0) return '您的交易记录还未被证实过不能抢单';
			if (db::dataCount('shop_order', "fuid='$this->id' AND qiang=2 AND addTime>=$t1") >= 10) return '每天最多抢10单';
			if (intval(db::one_one('shop_order', 'SUM(money)', "fuid='$this->id' AND qiang=2 AND addTime>=$t1")) >= 100000) return '每天最多抢100000';
		}
		if (db::exists('shop_order', "fuid='$this->id' AND status!=13 AND status!=20 AND qiang=2")) return '抢单还未完成。';
		if ($id = db::one_one('shop_order', 'id', "status=10 AND is_cs=1 AND qiang=2 AND (tuid<>'$this->id' OR fuid<>'$this->id') AND id='$id'")) {
			db::autocommit();
			if ($one = db::one('shop_order', '*', "id='$id'", false, true)) {
				if ($one['status'] == 10 && $one['is_cs'] == 1 && $one['qiang'] == 2) {
					/**
					 * 扣除激活码 如果扣除激活码失败 $next = false;
					 */
					$next = true;
					if (!$this->_isDebug) {
						if ($this->_checkNum >= 5 && db::update('member', 'checkNum=checkNum-5', "id='$this->id'")){//扣除激活码
							$next = true;
						} else {
							$next = false;
							db::autocommit(false);
							return '抢单需要5个激活码，完成后返回给您';
						}
					}
					if ($next) {//扣除激活码成功 创建买入
						$lixi = $one['money'] * 1.13;//13天利息
						$lixi = floor($lixi / 10 + 0.5) * 10;//
						$time = time();
						$buyDay = 0;
						$lixiFreezeDay = 2;//利息解冻时间2天 48小时
						$arr = array(
							'uid'            => $this->id,
							'allMoney'       => $one['money'],
							'yuMoney'        => 0,
						...
						if ($sid = db::insert('shop', $arr, true)) {
							$addTime = $time;
							$calTime = $time + 24 * 3600;
							$endTime = $time + 12 * 3600;
							if (db::update('shop_order', array(
								'fsid' => $sid, 
								'fuid' => $this->id, 
								'is_delay' => 0, 
								'delay_content' => '',
								...
								'calTime' => $calTime,
								'is_cs' => 0,
								'qiang' => 3), "id='$one[id]'")) {//OK 成功
								/**
								 * 冻结之前的帐号 和取消订单
								 */
								db::update('member', "status=1,is_status=1,dinfo='超时付款'", "id='$one[fuid]'");
								db::update('shop', "yuMoney=yuMoney+$one[money],status='101'", "id='$one[fsid]'");
								if (db::exists('shop_order', "fuid='$one[fuid]' AND (status=10 or status=11 or status=12)")){
								...
								db::commit(true);
								return true;
							} else {//修改失败，退回激活码并且数据滚回
								db::rollback(true);
								if (!$this->_isDebug) {
									db::update('member', 'checkNum=checkNum+5', "id='$this->id'");
								}
							}
						} else {//插入失败，退回激活码
							db::autocommit(false);
	...
	public static function overTime(){//超时订单处理
		...
		/**
		 * 大于10000进入抢单列表
		 * qiang=1 抢单列表 =2 允许抢单 拒绝抢单 改为0 并且调用 orderCancel1
		 */
		db::update('shop_order', 'is_cs=1,qiang=1,status=10', "is_cs=0 AND endTime<$time AND status in (10,11) AND money>=10000");
		
		//echo db::Sqlselect('shop_order', 'id,status', "status in (10,11,12) AND $time>endTime");exit;
		if ($list = db::select('shop_order', 'id,status', "is_cs=0 AND endTime<$time AND status in (10,11,12) ", false, 200)){
			foreach($list as $v){
