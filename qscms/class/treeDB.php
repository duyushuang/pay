...
class treeDB{
	...
	/*
	public static function moveTo1($tb, $cid, $pid = 0){//把cid以及下级  移动到pid下  
		$tb1 = $tb.'_tree';
		
		if (($rank = db::one_one($tb, 'rank', "id='$cid'")) !== false) {
			
			$list = self::childs_all($tb, $cid, 'id,rank');
			$ids = $cid;
			$Nrank = $rank - 1;
			$pid && $Prank = db::one_one($tb, 'rank', "id='$pid'");
			$Prank || $Prank = 0;
			db::update($tb, "rank=rank-$Nrank+$Prank", "id='$cid'");
			$sql = "INSERT INTO ".db::table($tb1)." SELECT pid,$cid cid,rank+1 rank,diff+1 diff FROM ".db::table($tb1)." WHERE cid='$pid'";
			db::query($sql);
			db::insert($tb1, array('pid' => $pid, 'cid' => $cid, 'rank' => $rank + 1, 'diff' => 1));
			foreach($list as $v){
				$ids.= ','.$v['id'];	
			}
			db::delete($tb1, "cid in ($ids)");
			foreach($list as $k => $v){
				if ($k == 0){
					$pid = $cid;	
				}
				db::update($tb, "rank=rank-$Nrank+$Prank", "id='$v[id]'");
				
				$sql = "INSERT INTO ".db::table($tb1)." SELECT pid,$cid cid,rank+1 rank,diff+1 diff FROM ".db::table($tb1)." WHERE cid='$v[id]'";
				db::query($sql);
				db::insert($tb1, array('pid' => $pid, 'cid' => $cid, 'rank' => $rank + 1, 'diff' => 1));	
			}
		...
		exit;
		if ($cid && self::childsCount_all($tb, $cid) > 0){//有下级
			$list = self::childs_all($tb, $cid, 'id,rank');
			...
			if (db::insert($tb1, array('pid' => $pid, 'cid' => $cid, 'rank' => 1, 'diff' => 1))) {
				db::update($tb, 'rank=1', "id='$cid'");
			}
			return true;
		}else return false;
	}
