...
class Securimage {
	...
	*/
	...
	function Securimage()
	{
		...
		$this->wordlist_file = s('data/words.txt');
		...
		$this->ttf_file      = s('images/fonts/AHGBold.ttf');
    ...
    $this->text_color       = new Securimage_Color(0x3d, 0x3d, 0x3d);
		...
		$this->background_directory = s('images/vcode/backgrounds');
	}
	 ...
	 */
	function check($code, $clear = true)
	{
		$this->code_entered = $code;
		$this->validate($clear);
		return $this->correct_code;
	}
	public static function checkForm($clear = true){
		$code = isset($_POST['vcode']) ? $_POST['vcode'] : '';
		if ($code) {
			$cls = new self();
			return $cls->check($code, $clear);
		}
		return false;
	}
	/**
	...
	function saveData()
	{
		...
		qscms::setcookie('securimage_code_value',strtolower($this->code));
		qscms::setcookie('securimage_code_ctime',time::$timestamp);
	 ...
	 *
	 */
	function validate($clear = true)
	{
		// retrieve code from session, if no code exists check sqlite database if supported.
		
		if (isset($_COOKIE['securimage_code_value']) && trim($_COOKIE['securimage_code_value']) != '') {
			if ($this->isCodeExpired(isset($_COOKIE['securimage_code_ctime']) ? $_COOKIE['securimage_code_ctime'] : 0) == false) { 
			  $code = $_COOKIE['securimage_code_value'];
			}
		...
		if ($code != '') {
			if ($code == $code_entered) {
			  ...
			  if ($clear) {
			  	qscms::setcookie('securimage_code_value', '');
			  	qscms::setcookie('securimage_code_ctime', '');
