			...
			if($(e0).attr('emptyRunReg')=='true'||($(e0).attr('emptyRunReg')=='false'&&$(e0).val())){
				...
				if($('#'+$(e0).attr('Message').split('|')[3]).val()==$(e0).val()){
					if($(e0).val()!=''){
						...
						else SetTipInfo(tid,'',fh_static_right);
						return true;
					} else {
						SetTipInfo(tid,'',fh_static_none);
					}
			...
			if($(e).attr('preg')){
				...
				//for(var k=0;k<pregs.length;k++){
					...
					if(sp[0]=='null'){
						...
						if(!f)$(e).attr('RegStr','/^[1-9]\\d{4,9}$/i');
						else $(e).attr('RegStr','/^$/');
					} else if(sp[0]=='phone') {
						if(!f)$(e).attr('RegStr','/(?:^1(?:(?:3[0-9])|(?:5[0-35-9])|(?:8[26-9]))\\d{8}$)|(?:^\\d{2,4}-\\d{7,8}$)|(?:^\\d{7,8}$)/');
						else $(e).attr('RegStr','/^$/');
					} else if(sp[0]=='telephone') {
						if(!f)$(e).attr('RegStr','/(?:^\\d{2,4}-\\d{7,8}$)|(?:^\\d{7,8}$)/');
						else $(e).attr('RegStr','/^$/');
					} else if(sp[0]=='mobilephone') {
						if(!f)$(e).attr('RegStr','/^1(?:(?:3[0-9])|(?:5[0-35-9])|(?:8[26-9]))\\d{8}$/');
						else $(e).attr('RegStr','/^$/');
					} else if(sp[0]=='postcode') {
						if(!f)$(e).attr('RegStr','/^\\d{6}$/');
						else $(e).attr('RegStr','/^$/');
					} else if(sp[0]=='image') {
						if(!f)$(e).attr('RegStr','/\\.(jpg|jpeg|png|gif)$/i');
						else $(e).attr('RegStr','/^$/');
					} else if(sp[0]=='syn') {
						$(e).attr('preg','syn');
						$(e).focus(function(){
