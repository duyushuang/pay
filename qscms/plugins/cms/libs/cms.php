...
$top_menu=array(
	...
	'cmsList'  => '{modelAlias}列表',
	'addCms'   => '添加{modelAlias}',
	'editCms'  => array('name' => '编辑{modelAlias}', 'hide' => true)
);
$modelName = 'slide';
...
if ($top = $var->getInt('gp_top') ) {
	if ($item = db::one('cms_slide', 'id,top', "id='$top'")) {
		db::update('cms_slide', 'top=1-top', "id='$top'");
...
switch ($method) {
	case 'cateList':
		//admin::getList('cms_slide_cate', '*', '', 'sort,addTime desc');
		if(form::is_form_hash()){
			extract($_POST);
			if($del || $sort){
				if($del){
					db::del_ids('cms_slide_cate', $del);
					db::del_keys('cms_slide', 'cid', $del);
					...
					if($count = form::array_equal($ids, $sort)){
						for($i=0; $i<$count; $i++){
							$id  = $ids[$i];
							$sid = $sort[$i];
							db::update('cms_slide_cate', array('sort'=>$sid),"id='$id'");
						}
		...
		$list = array();
		if($total=db::data_count('cms_slide_cate')){
			$list = db::get_list2('cms_slide_cate', '*', '', 'sort,addTime desc', $page, $pagesize);
	...
	case 'addCate':
		...
		if (form::is_form_hash()) {
			...
			if ($name && $ename) {
				if (db::insert('cms_slide_cate', array(
					'name'    => $name,
					'ename'   => $ename,
					'addTime' => $timestamp,
					'editTime' => $timestamp
	...
	case 'editCate':
		if (form::is_form_hash()) {
			extract(form::get3('name', 'ename'));
			if ($name && $ename) {
				if (db::update('cms_slide_cate', array(
					'name'     => $name,
					'ename'    => $ename,
					'editTime' => $timestamp
				), "id='$edit'")) {
		...
		$update = false;
		if ($item = db::one('cms_slide_cate', 'name,ename', "id='$edit'")) {
			extract($item);
			$update = true;
		}
	break;
	case 'cmsList':
				...
				if($del){
					...
					if($count = form::array_equal($ids, $sort)){
						for($i=0; $i<$count; $i++){
							$id  = $ids[$i];
							$sid = $sort[$i];
							db::update('cms_slide', array('sort'=>$sid),"id='$id'");
						}
		...
		$list = array();
		if($total = db::data_count('cms_slide', $where1)){
			//$list = db::get_list2('cms_slide', 'id,sort,title,ename,addTime,editTime,addIp,clicks', $where, 'cid,sort,addTime desc', $page, $pagesize);
			/*$list = array();
			$query = $db->query("select t1.id,t1.sort,t1.title,t1.ename,t1.addTime,t1.editTime,t1.addIp,t1.clicks,t1.top,t2.name from {$pre}cms_slide t1 left join {$pre}cms_slide_cate t2 on t2.id=t1.cid$where2 order by t1.cid,t1.sort,t1.addTime desc limit ".($page - 1) * $pagesize.','.$pagesize);*/
			$list = db::select('cms_slide|cms_slide_cate:id=cid', 'id,sort,addTime,editTime,addIp,clicks,top'.($showFieldName ? ','.$showFieldName : '').'|name', $where2, 't0.cid,t0.sort,t0.addTime DESC', $pagesize, $page);
	...
	case 'addCms':
		...
		if (form::is_form_hash()) {
			...
			if ($datas['cid']) {
				if (db::exists('cms_slide_cate', array('id' => $datas['cid']))) {
					...
					if (db::insert('cms_slide', $datas)) {
						db::update('cms_slide_cate', 'total=total+1', "id='$datas[cid]'");
		...
		$cates = array();
		foreach (db::get_list('cms_slide_cate', '*', '', 'sort,addTime desc', -1) as $v) {
			$cates[$v['name']] = $v['id'];
		}
	break;
	case 'editCms':
		...
		if (form::is_form_hash()) {
			...
			if ($datas['cid']) {
				if (db::exists('cms_slide_cate', array('id' => $datas['cid']))) {
					if ($oldCid = db::one_one('cms_slide', 'cid', "id='$editCms'")) {
						//$datas['content'] = addslashes(links_add::union(stripslashes($datas['content']), db::get_list('union_links', '`key`,url', '', 'sort,timestamp desc', -1)));
						$datas += array('editTime' => $timestamp);
						if (db::update('cms_slide', $datas, "id='$editCms'")) {
							if ($oldCid != $datas['cid']) {
								db::update('cms_slide_cate', 'total=total+1', "id='$datas[cid]'");
								db::update('cms_slide_cate', 'total=total-1', "id='$oldCid'");
							}
						...
						admin::show_message('要编辑的{modelAlias}不存在');
					}
				} else {
					admin::show_message('所属分类不存在！！');
				}
		...
		$update = false;
		if ($datas = db::one('cms_slide', '*', "id='$editCms'")) {
			//extract($item);
