...
class plugins{
	...
	private static function getRunPlugins(){
		...
		$datas = cache::get_array(self::$cacheName);
	...
	public static function call($type = 'article', $pluginName = 'cms', $m = 'run'){//echo func_num_args();exit;
		self::loadPlugins();
		$args = array();
		if (func_num_args() > 3) {
			$args = func_get_args();
		...
		if (isset(self::$plugins[$type])) {
			...
			template::config();
			if ($pluginName != '') {
				if (!empty(self::$plugins[$type][$pluginName]) && $plugin = self::$plugins[$type][$pluginName]) {
					if (method_exists($plugin, $m)) {
						template::initialize(PLUGIN_ROOT.$pluginName.D.'templates'.D, d(qscms::getCfgPath('/system/cacheDirRoot+/system/cacheDirs/tpl')).'plugins'.D.$pluginName.D);
						$datas = $plugin->$m($args);
						if (!empty($datas)) {
				...
				if (!in_array($filename, array('.', '..')) && filetype($file) == 'dir') {
					...
					if (file_exists($xmlFile) && file_exists($pluginFile)) {
						...
						if ($type == '' || ($type == 'installed' && $installed) || ($type == 'uninstalled' && !$installed)) {
							...
							$list[] = array(
								'folder'      => $filename,
								'title'       => empty($xmlArr['title'])       ? '' : trim($xmlArr['title']),
								'description' => empty($xmlArr['description']) ? '' : trim($xmlArr['description']),
								'author'      => empty($xmlArr['author'])      ? '' : trim($xmlArr['author']),
								'support'     => empty($xmlArr['support'])     ? '' : trim($xmlArr['support']),
								'marker'      => empty($xmlArr['marker'])      ? '' : trim($xmlArr['marker']),
								'version'     => empty($xmlArr['version'])     ? '' : trim($xmlArr['version']),
								'installed'   => $installed,
								'suspend'     => $suspend
							);
						}
	...
	private static function addRunPlugin($folder, $marker){
		$plugins = self::getRunPlugins();
		if (isset($plugins[$folder])) unset($plugins[$folder]);
	...
	public static function upCache(){
		$plugins = array();
		foreach (self::getPlugins() as $v) {
			if ($v['installed'] && !$v['suspend']) {
				$plugins[$v['folder']] = $v['marker'];
			}
		}
		cache::write_array(self::$cacheName, $plugins);
	}
		...
		if (file_exists($dir) && filetype($dir) == 'dir') {
			...
			if (file_exists($xmlFile) && file_exists($pluginFile)) {
				...
				if (file_exists($lockFile)) {
					...
					if ($uninstallFile) {
						$pluginName = $folder;
						$pluginType = empty($plugins[$folder]) ? '' : $plugins[$folder];
