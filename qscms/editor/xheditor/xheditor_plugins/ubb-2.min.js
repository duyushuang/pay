...
if(typeof postInsertDatas!='undefined')insertDatas=$.extend(insertDatas,postInsertDatas);
...
'<p align="$1">$2</p>');b=b.replace(/\[img\]\s*(((?!")[\s\S])+?)(?:"[\s\S]*?)?\s*\[\/img\]/ig,'<img src="$1" alt="" />');
...
b=b.replace(/\[img\s*=([^,\]]*)(?:\s*,\s*(\d*%?)\s*,\s*(\d*%?)\s*)?(?:,?\s*(\w+))?\s*\]\s*(((?!")[\s\S])+?)(?:"[\s\S]*)?\s*\[\/img\]/ig,function(b,a,c,j,e,q){b='<img src="'+q+'" alt="'+a+'"';e=e?e:!g(c)?c:"";g(c)&&(b+=' width="'+c+'"');g(j)&&(b+=' height="'+j+'"');e&&(b+=' align="'+e+'"');return b+" />"});b=b.replace(/\[emot\s*=\s*([^\]"]+?)(?:"[^\]]*?)?\s*\/\]/ig,'<img emot="$1" />');
...
"[align=$3]$4[/align]");for(c=0;3>c;c++)a=a.replace(/<(center)(?:\s+[^>]*?)?>(((?!<\1(\s+[^>]*?)?>)[\s\S])*?)<\/\1>/ig,"[align=center]$2[/align]");for(c=0;3>c;c++)a=a.replace(/<(p|div)(?:\s+[^>]*?)?\s+style\s*=\s*"((?:[^"]*?;)*\s*text-align\s*:[^"]*)"(?: [^>]+)?>(((?!<\1(\s+[^>]*?)?>)[\s\S]|<\1(\s+[^>]*?)?>((?!<\1(\s+[^>]*?)?>)[\s\S]|<\1(\s+[^>]*?)?>((?!<\1(\s+[^>]*?)?>)[\s\S])*?<\/\1>)*?<\/\1>)*?)<\/\1>/ig,function(){});a=a.replace(/<a(?:\s+[^>]*?)?\s+href=(["'])\s*(.+?)\s*\1[^>]*>\s*([\s\S]*?)\s*<\/a>/ig,
function(a,b,c,d){if(!c||!d)return"";a="url";c.match(/^mailto:/i)&&(a="email",c=c.replace(/mailto:(.+?)/i,"$1"));b="["+a;c!=d&&(b+="="+c);return b+"]"+d+"[/"+a+"]"});

a=a.replace(/<img[^>]*attach="(\d+)"[^>]*>/ig,'[attach]$1[/attach]');
a=a.replace(/<img[^>]*(insert\w+)="(\d+)"[^>]*>/ig,'[$1]$2[/$1]');

a=a.replace(/<img(\s+[^>]*?)\/?>/ig,function(a,b){var c=b.match(/\s+emot\s*=\s*(["']?)\s*(.+?)\s*\1(\s|$)/i);if(c)return"[emot="+c[2]+"/]";var c=b.match(l),d=b.match(/\s+alt\s*=\s*(["']?)\s*(.*?)\s*\1(\s|$)/i),f=b.match(e),i=b.match(m),k=b.match(/\s+align\s*=\s*(["']?)\s*(\w+)\s*\1(\s|$)/i),g="[img";if(!c)return"";d=""+d[2];if(f||i)d+=
...
a=a.replace(/((\s|&nbsp;)*\r?\n){3,}/g,"\r\n\r\n");a=a.replace(/^((\s|&nbsp;)*\r?\n)+/g,"");a=a.replace(/((\s|&nbsp;)*\r?\n)+$/g,"");for(c=1;c<=j;c++)a=a.replace("[\tubbcodeplace_"+c+"\t]",o[c]);a=a.replace(/<[^<>]+?>/g,"");var p={lt:"<",gt:">",nbsp:" ",amp:"&",quot:'"'};return a=a.replace(/&(lt|gt|nbsp|amp|quot);/ig,function(a,b){return p[b]})};
