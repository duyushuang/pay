...
function ubb2html($sUBB){
	$sHtml=$sUBB;
	global $emotPath,$cnum,$arrcode,$bUbb2htmlFunctionInit,$weburl2;
	$cnum=0;$arrcode=array();
	$emotPath = su('images/emot/');//·
	...
	function getSizeName($match)
	{
		...
		return '<span style="font-size:'.(isset($arrSize[$match[1]-1]) ? $arrSize[$match[1]-1] : $match[1].'px').';">';
	}}
	$sHtml=preg_replace_callback("/\[size\s*=\s*(\d+?)\s*\]/i",'getSizeName',$sHtml);
	$sHtml=preg_replace("/\[size\s*=\s*(\d+?)px\s*\]/i",'<span style="font-size:;$1px">',$sHtml);
	...
	$sHtml = preg_replace('/\[align\s*=\s*([^\]"]+?)(?:"[^\]]*?)?\s*\]/', '<p align="$1">', $sHtml);
	$sHtml = str_replace('[/align]', '</p>', $sHtml);
	...
	$sHtml = preg_replace('/\[mao\](\w+?)\[\/mao\]/', '<a id="$1"></a>', $sHtml);
	...
	$sHtml = preg_replace('/\[(table|tr|th|td)(.*?)\]/i', '<$1$2>', $sHtml);
	...
	$sHtml=preg_replace("/\[\/(table|tr|td|th)\]/i",'</$1>',$sHtml);
