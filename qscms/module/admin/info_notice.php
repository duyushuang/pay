...
$top_menu=array(
	...
	'cmsList'  => '公告列表',
	'addCms'   => '添加公告',
	'editCms'  => array('name' => '编辑公告', 'hide' => true)
);
$modelName = 'notice';
...
if ($top = $var->getInt('gp_top') ) {
	if ($item = db::one('cms_notice', 'id,top', "id='$top'")) {
		db::update('cms_notice', 'top=1-top', "id='$top'");
...
switch ($method) {
	case 'cateList':
		//admin::getList('cms_notice_cate', '*', '', 'sort,addTime desc');
		if(form::is_form_hash()){
			extract($_POST);
			if($del || $sort){
				if($del){
					db::del_ids('cms_notice_cate', $del);
					db::del_keys('cms_notice', 'cid', $del);
					...
					if($count = form::array_equal($ids, $sort)){
						for($i=0; $i<$count; $i++){
							$id  = $ids[$i];
							$sid = $sort[$i];
							db::update('cms_notice_cate', array('sort'=>$sid),"id='$id'");
						}
		...
		$list = array();
		if($total=db::data_count('cms_notice_cate')){
			$list = db::get_list2('cms_notice_cate', '*', '', 'sort,addTime desc', $page, $pagesize);
	...
	case 'addCate':
		...
		if (form::is_form_hash()) {
			...
			if ($name && $ename) {
				if (db::insert('cms_notice_cate', array(
					'name'    => $name,
					'ename'   => $ename,
					'addTime' => $timestamp,
					'editTime' => $timestamp
	...
	case 'editCate':
		if (form::is_form_hash()) {
			extract(form::get3('name', 'ename'));
			if ($name && $ename) {
				if (db::update('cms_notice_cate', array(
					'name'     => $name,
					'ename'    => $ename,
					'editTime' => $timestamp
				), "id='$edit'")) {
		...
		$update = false;
		if ($item = db::one('cms_notice_cate', 'name,ename', "id='$edit'")) {
			extract($item);
			$update = true;
		}
	break;
	case 'cmsList':
				...
				if($del){
					...
					if($count = form::array_equal($ids, $sort)){
						for($i=0; $i<$count; $i++){
							$id  = $ids[$i];
							$sid = $sort[$i];
							db::update('cms_notice', array('sort'=>$sid),"id='$id'");
						}
		...
		$list = array();
		if($total = db::data_count('cms_notice', $where1)){
			//$list = db::get_list2('cms_notice', 'id,sort,title,ename,addTime,editTime,addIp,clicks', $where, 'cid,sort,addTime desc', $page, $pagesize);
			/*$list = array();
			$query = $db->query("select t1.id,t1.sort,t1.title,t1.ename,t1.addTime,t1.editTime,t1.addIp,t1.clicks,t1.top,t2.name from {$pre}cms_notice t1 left join {$pre}cms_notice_cate t2 on t2.id=t1.cid$where2 order by t1.cid,t1.sort,t1.addTime desc limit ".($page - 1) * $pagesize.','.$pagesize);*/
			$list = db::select('cms_notice|cms_notice_cate:id=cid', 'id,sort,addTime,editTime,addIp,clicks,top'.($showFieldName ? ','.$showFieldName : '').'|name', $where2, 't0.cid,t0.sort,t0.addTime DESC', $pagesize, $page);
	...
	case 'addCms':
		...
		if (form::is_form_hash()) {
			...
			if ($datas['cid']) {
				if (db::exists('cms_notice_cate', array('id' => $datas['cid']))) {
					...
					if (db::insert('cms_notice', $datas)) {
						db::update('cms_notice_cate', 'total=total+1', "id='$datas[cid]'");
		...
		$cates = array();
		foreach (db::get_list('cms_notice_cate', '*', '', 'sort,addTime desc', -1) as $v) {
			$cates[$v['name']] = $v['id'];
		}
	break;
	case 'editCms':
		...
		if (form::is_form_hash()) {
			...
			if ($datas['cid']) {
				if (db::exists('cms_notice_cate', array('id' => $datas['cid']))) {
					if ($oldCid = db::one_one('cms_notice', 'cid', "id='$editCms'")) {
						//$datas['content'] = addslashes(links_add::union(stripslashes($datas['content']), db::get_list('union_links', '`key`,url', '', 'sort,timestamp desc', -1)));
						$datas += array('editTime' => $timestamp);
						if (db::update('cms_notice', $datas, "id='$editCms'")) {
							if ($oldCid != $datas['cid']) {
								db::update('cms_notice_cate', 'total=total+1', "id='$datas[cid]'");
								db::update('cms_notice_cate', 'total=total-1', "id='$oldCid'");
							}
		...
		$update = false;
		if ($datas = db::one('cms_notice', '*', "id='$editCms'")) {
			//extract($item);
