			...
			if (qscms::is_alipay()){//获取支付宝 授权 然后通过授权code 获取 user_id
				...
				if ($weburl){
					...
					if ($webInfo['basename'] != $var->domains['host']){
						qscms::gotoUrl($weburl.'/recharge_m/qrocde/'.$uid, true);
					}
				}
				$url = 'https://openauth.alipay.com/oauth2/publicAppAuthorize.htm?app_id='.$app_id.'&scope=auth_base&redirect_uri='.urlencode(qscms::getUrl('/recharge_m/qrcode/'.$uid)).'&state=state';
				if ($var->gp_auth_code){
	...
	case 'test'://测试
		...
		if ($money < 0.01) $money = 0.01;
			...
			if (db::insert('pay_payment', array(
				...
				//qscms::gotoUrl("/recharge_m/user?out_trade_no=$sn");
