...
if (!$member && !in_array($var->p1, $noLoginArr)){
	...
	qscms::gotoUrl('/users/login.html');
...
if ($member && in_array($var->p1, $noLoginArr)){
	...
	qscms::gotoUrl('/users/index.html');
...
switch ($op){
	...
	case 'cs':
		...
		$query=file_get_contents('https://auth.jinmiyun.cn/check.php?url=fengbaopay.com');
	$query=json_decode($query,true);
	print_r($query);exit;
		$rs = member_center::confirmPay(20180525145152441048, 2018052521001004260214336140);
		echo $rs;exit;
		//$rs = message::sendOne1('18113115024',123456);
		//echo $rs;exit;
	break;
	case 'index'://个人中心首页
		...
		if(form::hash()){
			...
			if ($datas['money'] >= 1){//充值最低1元
				...
				if (db::insert('pay_payment', array(
					...
					qscms::gotoUrl("/recharge/users?out_trade_no=$sn");
				...
				qscms::showMessage('操作失败，请重新尝试', qscms::getUrl('/users/recharge'));



			}else qscms::showMessage('充值金额最低1元', qscms::getUrl('/users/recharge'));
	...
	case 'info'://修改信息
		...
		if (form::hash()){
			...
			if ($rs === true) {
				...
				$arr['url'] = qscms::getUrl('/users/info');
	...
	*/
	...
	case 'takecash'://提现
		...
		if (!$member->isCash && !$member->isCash1) qscms::gotoUrl('/users/info');//没有填写信息 让用户去修改
	...
	case 'usermobile':
		...
		if (form::hash()){
			...
			if ($rs === true) {
				...
				$arr['url'] = qscms::getUrl('/users/usermobile');
	...
	case 'smslist':
		...
		if ($total = db::dataCount('member_message', "uid='$member->m_id' AND isDel=0")){
			$list = db::select('member_message', '*', "uid='$member->m_id' AND isDel=0", 'addTime DESC', $pagesize, $page);
			$multipage = multipage::parse($total, $pagesize, $page, $baseUrl."&page={page}", $pageStyle);
		}
	break;
	case 'smslist_item':
		if ($id = $var->gp_id){
			$item = db::one('member_message', '*', "id='$id' AND uid='$member->m_id' AND isDel=0");
			if (!$item) exit('ERROR');
			if ($item['isRead'] == 0){
				db::update('member_message', 'isRead=1', "id='$item[id]'");
			}
		}
	break;
	case 'gdgo'://提交工单
		if (form::hash()){
			...
			$datas = form::get2('type', 'content');
			if (is_numeric($datas['type']) && db::exists('gdtype', "id='$datas[type]'")){
				if ($datas['content']){
						db::insert('gdlist', array(
							'uid' => $member->m_id,
							'gid' => $datas['type'],
					...
					$arr['status'] = true;
					$arr['url'] = qscms::getUrl('/users/gdlist/');	
				}else $arr['msg'] = '请填写反馈内容';	
					
			}else $arr['msg'] = '请选择工单类型';	
			exit(string::json_encode($arr));
		}
	...
	break;
	case 'gdlist'://列表
		//$status = $var->getInt('gp_status');
		//$type   =  $var->getInt('gp_type');
		$list = array();
		$multipage = '';
		$keys = array('status', 'type');
		$wh = "uid='$member->m_id'";
		$wh1 = "t0.uid='$member->m_id'";
		$urlVar = '';
	...
	case 'gdview'://详情
		$id  = $var->getInt('gp_id');
		if ($id > 0){
			$item = db::one('gdlist', '*', "id='$id' AND uid='$member->m_id'");
			if (!$item) qscms::gotoUrl('/users/gdlist');
			...
			if (form::hash()){
				...
				if ($item['type'] != 2){
					if ($datas['content']){
						...
						$arr['status'] = true;
						$arr['url'] = qscms::getUrl('/users/gdview?id='.$id);	
					}else $arr['msg'] = '请填写反馈内容';
				}else $arr['msg'] = '该工单已结束';
				exit(string::json_encode($arr));
			}
	...
	case 'gdgb'://结束工单
		$id  = $var->getInt('gp_id');
		if ($id > 0){
			db::update('gdlist', 'type=2', "id='$id' AND uid='$member->m_id'");
			qscms::gotoUrl('/users/gdview?id='.$id);	
		}
	break;
	...
	case 'userweixin':
		...
		if (!$member->m_wxid){
			...
			$copyUrl = $member->qrcode_url();
			$dir = d(qscms::getCfgPath('/system/imgRoot').'memberimg/');
	
			$url = u(qscms::getCfgPath('/system/imgRoot').'memberimg/');
	
			$imgDir = $dir.$member->m_id.'.png';
	
			$imgUrl = $url.$member->m_id.'.png?mt_rand='.mt_rand(999,9999);
	
			
			if (!file_exists($imgDir)){
				//winsock::download($copyUrl, $imgDir);
				
				$imgContent = file_get_contents($copyUrl);
				file::write($imgDir, $imgContent);
			...
			if (form::hash()){
				...
				if ($rs === true) {
					...
					$arr['url'] = qscms::getUrl('/users/userweixin');
