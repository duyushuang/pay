<?php
if (!defined('_PHP55')) define('_PHP55', version_compare(PHP_VERSION, '5.5','>='));
...
function __rewrite(){
	...
	foreach($r_rewrite as $v){
		if(preg_match($v[0],$r_uri,$matches)){
			$r_uri=$v[1];
			if (_PHP55) {
				$r_uri = preg_replace_callback('/\$(\d+)/', function($ms) use($matches) {
					return !empty($matches[$ms[1]]) ? $matches[$ms[1]] : '';
				}, $r_uri);
			} else {
				$r_uri = preg_replace('/\$(\d+)/e','!empty($matches[$1]) ? $matches[$1] : \'\'',$r_uri);
			}
			$matching=true;
			break;
		}
...
if($r_q=$_SERVER['QUERY_STRING']){
	if(substr($r_q,0,$r_marker_len)===$r_marker){
		...
		if($r_){
			//if not null
			$r_ = '/'.$r_;
			define('REWRITE_URL', $r_);
			$r_dir = dirname(__FILE__);
			$r_d   = DIRECTORY_SEPARATOR;
			//if exists out
			$r_include = str_replace('/',$r_d,$r_dir.iconv('utf-8', 'gbk', $r_));
			...
			if(file_exists($r_include)){
				if(filetype($r_include)=='file'){
					...
					if($r_extension){
						...
						if($r_extension=='php' || $r_extension=='inc'){
				...
				//exe rewrite
				$r_rewrite_file=dirname(__FILE__).DIRECTORY_SEPARATOR.'./qscms/static/data/rewrite.qscms';
			...
			unset($r_);
			include(dirname(__FILE__).DIRECTORY_SEPARATOR.'index.php');//index
		}
		/*rewrite end*/
	} else header("HTTP/1.1 404 Not Found");
} else include(dirname(__FILE__).DIRECTORY_SEPARATOR.'index.php');
?>